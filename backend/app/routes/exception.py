...
def create_exception(prof_id: str, excep: schema_exception.ExceptionBase, db: Session = Depends(get_db)):#, user_info = Depends(RolesValidator(["Profesional"]))):
...
def get_exception(prof_id: str, db: Session = Depends(get_db)):#, user_info = Depends(RolesValidator(["Profesional"]))):
    return ExceptionController(db= db).getException(prof_id)

@router.put('', tags=['Exceptions'])
def update_exception(prof_id: str, update:schema_exception.ExceptionUpInfo, db: Session = Depends(get_db)):#, user_info = Depends(RolesValidator(["Profesional"]))):
...
def delete_exception(prof_id: str, delete:schema_exception.ExceptionDel, db:Session = Depends(get_db)):#, user_info = Depends(RolesValidator(["Profesional"]))):
