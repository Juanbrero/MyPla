...
def create_specific_day(prof_id: str, specific:schema_topic_specific.TopicSpecificCr1, db:Session = Depends(get_db)):#, user_info = Depends(RolesValidator(["Profesional"]))):
    """
...
def get_specific(prof_id: str, db:Session = Depends(get_db)):#, user_info = Depends(RolesValidator(["Profesional"]))):
    """
...
def update_specific(prof_id: str, specific:schema_topic_specific.TopicSpecificDay,  db:Session = Depends(get_db)):#, user_info = Depends(RolesValidator(["Profesional"]))):
    """
...
def delete_specific(prof_id: str, specific:schema_topic_specific.TopicSpecificDeleteCtrl,  db:Session = Depends(get_db)):#, user_info = Depends(RolesValidator(["Profesional"]))):
    """
    Actualizar dia especifico de un profesional
    """
