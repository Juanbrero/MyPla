...
from app.bd.schemas import  schema_topic, schema_prof_topic, schema_prof
...
@router.post("/professionals/{prof_id}",
...
async def get_topics(prof_id:str, db:Session =Depends(get_db)):
    ...
    """
    prof = schema_prof.ProfessionalID(prof_id= prof_id)
...
@router.put('/professionals/{prof_id}', response_model=Union[Info,Errors], tags=['Prof Topic'])
def update_price_professional(prof_id:str, topic: schema_prof_topic.ProfessionalTopicCreate, db: Session = Depends(get_db)):
    prof_topic = schema_prof_topic.ProfessionalTopic(**topic.dict(), prof_id=prof_id)
    return crud_prof_topic.update_price(db, prof_topic)
