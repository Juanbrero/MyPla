...
class TestRepositoryInsert(TestCase):
    ...
    def test_create_not_topic(self):
        ...
        self.assertEqual(error_message(response.body), 'GRIEGO is not of the Professional')

    
    def test_create_with_invalid_topics(self):
        print('Insert one topic valid, one not exist')
        ...
        self.assertEqual(response.status_code, 404)
        self.assertEqual(error_message(response.body), 'FRANCES is not of the Professional')
        del_topic('FRANCES')
...
class TestRepositoryUpdate(TestCase):
    def setUp(self):
        self.recurrentC = RecurrentController(get_db_repository())
        self.prof_id = str(uuid())
        create_user(self.prof_id)
 ...
 # Update
    ...
    def test_add_topic_invalid(self):
        ...
        response = self.topic_recurrentC.addTopic(topic_recurrentS)
        self.assertEqual(response.status_code, 404)
        self.assertEqual(error_message(response.body), 'SUECO is not of the Professional')
...
class TestRepositoryDelete(TestCase):
...
class TestEndpointPUT(TestCase):
    ...
    def test_update_topic_add_invalid(self):
        ...
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.json()['error']['message'], 'API is not of the Professional')
