...
const CeldaHoraMyStudent = ({ dia, hora, eventosDelDia, onClick }) => {
  const { reservations = [] } = eventosDelDia || {};
  const todosEventos = [...reservations];
  const { evento, posicion } = eventoEnHora(todosEventos, dia, hora);

  let claseCelda = 'celda-hora';
  if (evento) {
    claseCelda += ' celda-hora-ocupada celda-reserve';
    if (posicion === 'start') {
...
export default CeldaHoraMyStudent;
